...
BRONZE $5/week: up to 5 tips a week — 3 standard tips (across NRL + AFL combined) + 1 Same Game Multi. 3+ stars, 2-3 dot points reasoning.
SILVER $10/week: up to 6 tips — the sharpest of the round, HIGH CONFIDENCE only (4+ stars / 85%+), 3-4 dot points, 1 AFL SGM + 1 NRL SGM, player props.
GOLD $20/week: up to 8-10 tips (4+ stars) + 2 AFL SGMs + 2 NRL SGMs + 1 cross-sport multi, best bookie value alerts, line movement alerts, weekly AI report.
...
📋 WEEKLY TIPS SUMMARY
───────────────────────────────────────
Total tips: [X] | Bronze: up to 5 (3+1SGM) | Silver: up to 6 (4★+, 1 AFL SGM + 1 NRL SGM) | Gold: up to 8-10 + 2 AFL SGMs + 2 NRL SGMs + cross-sport multi
...
📊 Reasoning: • [point] • [point] • [point]
─────────────────────────
[3 STANDARD BRONZE TIPS — the 3 best of the round across NRL + AFL — PLUS 1 SAME GAME MULTI (3-4 legs)]
─────────────────────────
...
─────────────────────────
[UP TO 6 SILVER TIPS (high confidence 4★+ / 85%+) + 1 AFL SGM + 1 NRL SGM (3-4 legs each) + PLAYER PROPS]
─────────────────────────
...
[2 AFL SGMs + 2 NRL SGMs (3-4 legs each) + 1 CROSS-SPORT MULTI]
[BEST BOOKMAKER VALUE ALERTS]
[LINE MOVEMENT ALERTS]
[WEEKLY AI ANALYSIS REPORT with NRL/AFL/Bookmaker analysis + Next Week Watchlist]
...
3. ODDS: every price is from Scout's data, at or above $1.50, and SGMs land in the $3–$8 band.
4. SGM LEGS: 3–4 legs only; AFL legs use locked-in starters; no margin/hit-out legs; no two legs that can't both be true.
...
const SCORECARD_PROMPT = `You are The Scorecard, the fourth and final agent in The Punters Den tipping pipeline.
Review this week's tip results and write a full transparency report for all channels.

STAKING — FLAT 1 UNIT: The Punters Den uses flat staking. EVERY tip is exactly 1 unit, regardless of confidence stars or odds. 1 unit = $10. Do not vary stake by confidence. SGMs are also 1 unit each. This keeps results honest, transparent and easy for members to follow. Calculate profit/loss on this flat 1-unit basis: a winning tip at $1.80 returns 0.8 units profit; a losing tip is -1 unit. ROI = total units profit/loss ÷ total units staked × 100.

CALCULATE:
1. Weekly record: tips won/lost, win rate %, SGMs won/lost, SGM hit rate, units profit/loss (flat 1 unit per tip)
2. Season record: total wins/losses, season win rate %, season ROI %, best sport, best market type, SGM season hit rate

WRITE 3 POSTS SEPARATED CLEARLY:

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
POST 1 — PUBLIC CHANNEL
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🐕 THE PUNTERS DEN
Week [X] Scorecard 📊
Another week done legends 🦘 | Full transparency report below 👇
─────────────────────────
WEEK [X] RESULTS:
🏉 NRL: [✅/❌] Tip 1: [Selection] — [Result] [repeat all]
🏈 AFL: [✅/❌] Tip 1: [Selection] — [Result] [repeat all]
─────────────────────────
📈 WEEK [X] STATS: Record: [W]W—[L]L | Win Rate: [X]% | Units: [+/-X]
📈 SEASON STATS: Record: [W]W—[L]L | Win Rate: [X]% | ROI: [+/-X]%
─────────────────────────
🔒 WANT THE FULL BREAKDOWN? [subscription CTA]
━━━━━━━━━━━━━━━━━━━━━━━━
🐕 The Punters Den | "The den finds the edge 🐕"
⚠️ Gamble responsibly — 18+ only
━━━━━━━━━━━━━━━━━━━━━━━━

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
POST 2 — BRONZE + SILVER
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[Full result breakdown for every tip with score, what happened, dot point analysis]
[SGM results with each leg]
[Weekly + Season stats]
[Next week preview tease]
━━━━━━━━━━━━━━━━━━━━━━━━
🐕 The Punters Den | Good luck next week legends 🦘
⚠️ Gamble responsibly — 18+ only
━━━━━━━━━━━━━━━━━━━━━━━━
...
POST 3 — GOLD VIP
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[All Bronze/Silver content PLUS full AI weekly analysis: NRL analysis, AFL analysis, bookmaker analysis, what we learned, next week watchlist]
━━━━━━━━━━━━━━━━━━━━━━━━
🐕 The Punters Den | You're in the den. Back yourself 👑🦘
⚠️ Gamble responsibly — 18+ only
━━━━━━━━━━━━━━━━━━━━━━━━

Be honest about losses. End on positive note. Confirm all 3 posts ready for the website member area.`;
...
function OutputBox({ text, onCopy, copied }) {
  return (
    <div>
    ...
    <div>
      ...
      {output && (
        <div>
          ...
          <OutputBox text={output} onCopy={copy} copied={copied} />
        </div>
      )}
    </div>
  );
            ...
            <div key={t.key}>
              <div style={{ background: "#17212b", border: `1px solid ${t.border}33`, borderRadius: 10, overflow: "hidden" }}>
                <div style={{ background: t.bg, borderBottom: `1px solid ${t.border}44`, padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button onClick={() => copy(t.key, posts[t.key])} style={{
...
function ScorecardAgent({ prefillAnalyst }) {
  ...
  const [posts, setPosts] = useState({ public: "", bronze: "", gold: "" });
  const [activePost, setActivePost] = useState("public");
  const [loading, setLoading] = useState(false);
  const [step, setStep] = useState(-1);
  const [error, setError] = useState("");
  const [copied, setCopied] = useState("");
  ...
  const today = new Date().toLocaleDateString("en-AU", { weekday: "long", year: "numeric", month: "long", day: "numeric", timeZone: "Australia/Sydney" });
  const steps = ["📋 Reading results...", "🧮 Calculating weekly stats...", "📈 Updating season record...", "🆓 Writing Public post...", "🥉🥈 Writing Bronze/Silver post...", "👑 Writing Gold VIP report..."];

  const splitPosts = (text) => {
    const p = { public: "", bronze: "", gold: "" };
    const pub = text.match(/🐕 THE PUNTERS DEN\s*\nWeek \d+ Scorecard[\s\S]*?(?=🐕 THE PUNTERS DEN\s*\nWeek \d+ Full|$)/i);
    const bron = text.match(/🐕 THE PUNTERS DEN\s*\nWeek \d+ Full Breakdown[\s\S]*?(?=🐕 THE PUNTERS DEN\s*\nWeek \d+ Den VIP|$)/i);
    const gold = text.match(/🐕 THE PUNTERS DEN\s*\nWeek \d+ Den VIP[\s\S]*/i);
    p.public = pub?.[0]?.trim() || text;
    p.bronze = bron?.[0]?.trim() || "";
    p.gold = gold?.[0]?.trim() || "";
    if (!p.bronze && !p.gold) p.public = text;
    return p;
  };
  ...
  const postTabs = [
    { key: "public", label: "🆓 Public", color: "#aaa", border: "#333", bg: "#141414" },
    { key: "bronze", label: "🥉🥈 Bronze/Silver", color: "#cd7f32", border: "#cd7f32", bg: "#1a1200" },
    { key: "gold", label: "👑 Gold VIP", color: "#ffd700", border: "#ffd700", bg: "#1a1500" },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ background: "#111", border: "1px solid #222", borderRadius: 12, padding: 24, marginBottom: 16 }}>
        ...
        <p style={{ margin: "0 0 10px", color: "#aaa", fontSize: 13 }}>Paste last week's season stats to keep cumulative record rolling.</p>
        <Textarea value={season} onChange={setSeason}
          placeholder={"Season record: 18W 9L (67% win rate)\nSeason ROI: +12.4%\nSGM hit rate: 22%\nBest sport: NRL\nBest market: H2H"}
          rows={5} />
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 32 }}>
              {[
                ...
                { id: "scorecard", icon: "📈", name: "Scorecard Agent", sub: "Run every Monday", desc: "Reviews all results, calculates weekly and season stats, writes 3 transparency posts for your channels.", color: "#22c55e", border: "#15803d", bg: "#0d2010", tags: ["Weekly Record", "Season ROI", "SGM Hit Rate", "Transparency Posts"] },
              ].map((agent, idx) => (
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {[
                    ...
                    { day: "Monday", icon: "📊", tasks: ["Enter results", "Run Scorecard", "Post transparency reports", "Save season stats for next week"] },
                  ].map(d => (
